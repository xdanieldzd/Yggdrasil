    ...
    public abstract class CompressedStream : MemoryStream
    {
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            if (CompressionMode == CompressionMode.Decompress)
            {
                ...
                {
                    throw new CompressedStreamException(string.Format("{0} decompression failed.", this.GetType().Name), ex);
                }
            }
            else if (CompressionMode == CompressionMode.Compress)
            {
                ...
                {
                    throw new CompressedStreamException(string.Format("{0} compression failed.", this.GetType().Name), ex);
                }
            }
        }
