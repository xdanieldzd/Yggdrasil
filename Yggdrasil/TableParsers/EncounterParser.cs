...
namespace Yggdrasil.TableParsers
{
    ...
    [PrioritizedDescription("Enemy Encounters", 0)]
    public class EncounterParser : BaseParser
    {
        ...
        {
            string description = (typeof(EncounterParser).GetCustomAttributes(false).FirstOrDefault(x => x is DescriptionAttribute) as DescriptionAttribute).Description;
